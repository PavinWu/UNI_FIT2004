		...
		if isCam[currentLocNode[0]]:		# go one at a time when going next vertex. if not put Cam node in the way of path -> will not go pass cam
			...
			for u in adjList[currentLocNode[0]]:
				if not u[2] and u[0] is not None:			# not TOLL and not Terminal 
					...
					elif finalList[u[0]] != -1 and discoveredHeap(finalList[u[0]]) > currentLocNode[1]+u[1]:
		...
		if currentLoc == len(adjList)-1:	# same edge
			...
			if currentLoc > len(adjList):
				for i in range(len(adjList), currentLoc):
					adjList.append([(None, 0, False)])	# no more connected node
			adjList.append([(nextLoc, distance, TOLL)])
